...
class CycleGANModel(tf.keras.Model):
    ...
    def train_step(self, image_data):
        ...
        with tf.GradientTape(persistent=True) as gradient_tape:
            ...
            discriminator_1_loss = self.losses_functions.discriminator_loss(real_image_evaluation_1,
                                                                            fake_image_evaluation_1)
            discriminator_2_loss = self.losses_functions.discriminator_loss(real_image_evaluation_2,
                                                                            fake_image_evaluation_2)

        generator_1_gradients = gradient_tape.gradient(total_generator_1_loss,
                                                       self.architecture.generator_1.trainable_variables)
        generator_2_gradients = gradient_tape.gradient(total_generator_2_loss,
                                                       self.architecture.generator_2.trainable_variables)

        discriminator_1_gradients = gradient_tape.gradient(discriminator_1_loss,
                                                           self.architecture.discriminator_1.trainable_variables)
        discriminator_2_gradients = gradient_tape.gradient(discriminator_2_loss,
        ...
        discriminator_1_grads_and_vars = zip(discriminator_1_gradients,
                                             self.architecture.discriminator_1.trainable_variables)
        discriminator_2_grads_and_vars = zip(discriminator_2_gradients,
                                             self.architecture.discriminator_2.trainable_variables)
